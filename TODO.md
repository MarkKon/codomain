...
- [ ] Add back and forth buttons for the markdown (and update the buffer accordingly) -> Also add support for this using mac-native back and forth (so it works e.g. with the mouse buttons etc.)
- [ ] Keep "focus" on the nvim instance so that edits etc. always happen directly, i.e. that even after clicking on the md preview one can directly use the nvim input without having to click on it again.
- [ ] Explore how to best render md. Currently mostly a custom implementation? But I guess in the end, it would be best to just use a dependency?
- [ ] Explore possibility to use themes?
